...
///
/// This function creates a smoltcp interface, obtains an IP via DHCP,
/// and then queries the NTP server.
pub fn sync_time_with_device(device: &mut WifiDevice<'_>) -> Result<NtpTime, &'static str> {
    ...
    while !got_ip && timeout_counter < 100 {
        ...
        if let Some(dhcpv4::Event::Configured(config)) = dhcp_socket.poll() {
            info!("NTP: DHCP configured: {:?}", config.address);
    ...
    info!("NTP: Got IP address: {}", our_ip);
    ...
    let timestamp = Instant::from_millis((timeout_counter as i64 + 1) * 100);
    iface.poll(timestamp, device, &mut sockets);

    // Wait for response
    info!("NTP: Waiting for response...");
    let mut response_timeout = 0u32;
    let mut ntp_response = [0u8; 48];
    let mut got_response = false;

    while !got_response && response_timeout < 50 {
        let timestamp = Instant::from_millis((timeout_counter + response_timeout + 2) as i64 * 100);
        iface.poll(timestamp, device, &mut sockets);
