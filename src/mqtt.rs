...
use esp_mbedtls::{Certificates, Mode, Session, TlsVersion, TlsReference};
...
pub fn publish_button_feedback<'a, 'd>(
    ...
    info!("MQTT: Preparing to publish feedback for button {}", button_id);
    ...
    info!("MQTT: Connecting TCP to {}...", remote_endpoint);
    {
        ...
        socket.connect(iface.context(), remote_endpoint, 45678).map_err(|_| "TCP Connect failed")?;
    }
    ...
    
    {
        ...
        for _ in 0..1000 { // 10 second timeout approx
            stream.poll();
        ...
        if connected {
            ...
            const CA_CERT: &[u8] = concat!(include_str!("../mosquitto/config/certs/ca.crt"), "\0").as_bytes();
            ...
            match Session::new(
                ...
                Ok(mut session) => {
                     ...
                     if let Err(e) = session.connect() {
                        ...
                        connection_result = perform_mqtt_actions(&mut session, client_id, button_id, timestamp);
                     }
                },
                Err(e) => {
...
fn perform_mqtt_actions<T: Read + Write>(
    ...
    let payload_str = alloc::format!("Button {} pressed at {}", button_id, timestamp);
